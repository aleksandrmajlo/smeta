...
class Services extends Section implements Initializable
{
    ...
    {
        $this->addToNavigation()->setPriority(600)->setIcon('fa fa-info');
    }
    ...
    public function onDisplay($payload = [])
    {
        $columns = [
            ...
            AdminColumnEditable::text('price')->setLabel('Цена'),
            AdminColumn::text('price_type', 'Тип цены')->setWidth('50px')->setHtmlAttribute('class', 'text-center'),
    ...
    public function onEdit($id = null, $payload = [])
    {
        ...
        $form = AdminForm::card()->addBody([
            AdminFormElement::columns()->addColumn([
                ...
                AdminFormElement::select('type', 'Тип услуги')->setOptions($types)->required(),
                AdminFormElement::number('price', 'Цена'),
                AdminFormElement::select('price_type', 'Тип цены')->setEnum(['руб.','%']),
                AdminFormElement::checkbox('default', 'Применять по умолчанию(цена считаться не  будет)'),
            ])
        ]);
